...
- **One personal remote** (Eric's own account, never Opus): `origin` → GitHub
  `https://github.com/ericfu-tianchi/twist-solver`. (GitLab was briefly mentioned then retracted —
  **GitHub is the only home**.)
...
- **Recommended: GitHub Pages** — already on his GitHub, nothing to configure but a toggle. Repo must
  be **public** (it is) → GitHub → repo → Settings → Pages → "Deploy from a branch" → `main` / `/ (root)`
  → Save. URLs: Desktop `https://ericfu-tianchi.github.io/twist-solver/` · Mobile `…/mobile.html`.
- Alternatives (all free, connect the GitHub repo, **no build command**, output dir = `/`, auto-deploy
  on push, free custom domain): **Cloudflare Pages** (fastest CDN) · **Netlify** · **Vercel**.
